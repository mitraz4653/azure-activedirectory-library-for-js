  ...
  */
    ...
    AuthenticationContext.prototype.logOut = function () {
        ...
        if (this.config.logOutUri) {
            ...
            if (this.config.tfp) {
              tenant = this.config.tfp + '/' + tenant;
            }

            tenant += this.config.signInPolicy ? ('/' + this.config.signInPolicy) : '';

            if (this.config.postLogoutRedirectUri) {
                logout = 'post_logout_redirect_uri=' + encodeURIComponent(this.config.postLogoutRedirectUri);
            }

            urlNavigate = this.instance + tenant + '/oauth2/' + (this.config.signInPolicy? 'v2.0/' : '') + 'logout?' + logout;
        }
    ...
    */
    ...
    AuthenticationContext.prototype._getNavigateUrl = function (responseType, resource) {
        ...
        var urlNavigate = this.instance + tenant + '/oauth2/' + (this.config.signInPolicy? 'v2.0/' : '') + 'authorize' + this._serialize(responseType, this.config, resource) + this._addLibMetadata();
